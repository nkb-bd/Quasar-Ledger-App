<template>
  <q-layout view="lHh Lpr lFf" >
    <q-header >
      <q-toolbar>
        ...
        <q-toolbar-title>
          ...
          <q-btn  flat exact to="/" >  Ledger App
          </q-btn>
        ...
        <div v-if="!userDetails.userId">
          <q-btn  exact to="/auth"  outline clickable v-ripple    icon-right="account_box" label="Login" >
          </q-btn>
        </div>
        <div  v-if="userDetails.userId">
          <q-btn  @click="logoutUser" outline clickable v-ripple  icon-right="exit_to_app"  >
            Logout
          </q-btn>
        </div>
      ...
      <div class="q-px-lg q-pt-md q-mb-md">
        ...
        <div class="text-subtitle-1">
          {{todaysDate}} {{Object.keys(userDetails).length }}
        </div>
      </div>
      <q-img
      ...
      />
      <div class="absolute-bottom-right z-max" > {{loggedInUser}}      @created by lukman.nakib@gmail.com
      </div>
    </q-header>
    ...
    >
      ...
      <q-img class="absolute-top" :src=" 'statics/mountain.jpg' "style="height: 185px">
        <div class="absolute-bottom bg-transparent">
          <q-avatar size="56px" class="q-mb-sm">
            <img src="https://cdn.quasar.dev/img/boy-avatar.png">
          </q-avatar>
          <div v-if="userDetails.userId" class="text-weight-bold"> {{userDetails.name}}</div>
          <div>@lukman.nakib</div>
        </div>
      </q-img>
